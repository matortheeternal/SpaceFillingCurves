...
public class MooreCurve {
	...
	public static void main(String[] args) {
		...
		System.out
				.println("Enter the iteration of the Moore Curve you want to generate (n >= 1)");
	...
	public static void DrawMooreCurve(UserInterface ui, int iteration) {
		// initialize canvas
		int width = ui.getWidth(); // canvas x size
		int height = ui.getHeight(); // canvas y size
		int hsl = getMooreSize(iteration); // Moore curve side length in line units
		...
		// prepare for drawing
		int state = 0;
		int d = (width < height) ? (width - 2*xi) / hsl : (height - 2*yi) / hsl; // length of a Moore line unit on our canvas
		int x = xi + ((hsl - 1)/2)*d; // x coordinate
		int y = height - yi; // y coordinate
		int dx = x; // destination x for line
		int dy = y; // destination y for line
		
		// prepare curve
		String curve = generateMooreCurve(iteration);
...
//		ui.setImage(img);
		File outputfile = new File("moore-"+iteration+".png");
		try {
	...
	public static String generateMooreCurve(int iteration) {
		String curve = "-AFA+F+AFA";
	...
	private static int getMooreSize(int iteration) {
		int result = 1;
		for (int i = 0; i < iteration; i++) {
			result *= 2;
			result++;
